    ...
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 className="txt mt-2 flex items-center gap-3 text-3xl font-bold tracking-tight md:text-4xl">
          {Icon && (
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 text-white shadow-lg shadow-blue-600/20">
              <Icon size={20} />
            </span>
          )}
          {title}
...
const SEV = {
  ...
  Open: 'border-blue-500/40 bg-blue-500/15 text-blue-400',
...
export function Textarea(props) {
  return (
    <textarea
      {...props}
      className={`bd tile txt w-full rounded-lg border p-3 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500/40 ${
        props.className || ''
      }`}
    />
  )
...
export function Input(props) {
  return (
    <input
      {...props}
      className={`bd tile txt w-full rounded-lg border px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500/40 ${
        props.className || ''
      }`}
    />
  )
