...
function DownloadCard({ icon: Icon, name, tagTone, desc, hint, accent, pins, toast }) {
  ...
  const go = () => {
    ...
    // The PIN must have been generated by an analyst — random strings are
    // rejected. If the local store is empty (visitor not logged in) we
    // tell them so explicitly instead of silently failing.
    if (!pins || pins.length === 0) {
      toast({
        type: 'error',
        title: 'No PIN available',
        body: 'Log in and create a PIN under Pins before downloading the scanner.',
      })
      return
    }
    ...
    if (!match) {
      toast({
        type: 'error',
        title: 'PIN not found',
        body: 'This PIN was not generated by an analyst on this account. Ask them to verify it.',
      })
      return
    }
    ...
    // 1) Session manifest — the scanner reads the PIN from it.
    const sessionLink = document.createElement('a')
    sessionLink.href = URL.createObjectURL(
      new Blob([JSON.stringify(session, null, 2)], { type: 'application/json' }),
    )
    sessionLink.download = `ZeroTraceScan-${match.pin}.zerotrace`
    sessionLink.click()
    URL.revokeObjectURL(sessionLink.href)

    // 2) The actual scanner .exe — deploy.yml ships it at /ZeroTrace.exe.
    if (name === 'Windows') {
      ...
      toast({
        ...
        body: `ZeroTrace.exe + ZeroTraceScan-${match.pin}.zerotrace`,
      })
    } else {
      toast({
        type: 'info',
